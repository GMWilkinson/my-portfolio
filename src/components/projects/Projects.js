...
class Intro extends React.Component {
  ...
  render() {
    const {projectFrom} = this.state;
    return (
      <article className="project">
        <div>
          <h2 className="" id="projects">{this.state.projectFrom.toUpperCase()} Projects</h2>
        </div>
        <div style={{display: 'flex', justifyContent: 'space-around'}}>
          <span onClick={() => this.changeProjectType('commercial')}>Commercial Projects</span>
          <span onClick={() => this.changeProjectType('ga')}>GA Projects</span>
        </div>
        <div>
          <ProjectIntro projectFrom={projectFrom}/>
          <h2 onClick={this.toggle}>Projects</h2>
          {projectFrom ===  'commercial' ?
          <div>
